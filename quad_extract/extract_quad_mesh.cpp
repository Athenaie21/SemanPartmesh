...
int main(int argc, char *argv[])
{
    ...
    double gradient_size      = (argc > 4) ? std::atof(argv[4]) : 30.0;
    double stiffness          = (argc > 5) ? std::atof(argv[5]) : 5.0;
    bool   direct_round       = (argc > 6) ? (std::atoi(argv[6]) != 0) : false;
    int    iters              = (argc > 7) ? std::atoi(argv[7]) : 5;
    int    local_iters        = (argc > 8) ? std::atoi(argv[8]) : 5;
    ...
    for (int i = 0; i < nf; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            int uv_idx = FUV(i, j);
            triMesh.uvTris[i].uvs[j].x[0] = UV(uv_idx, 0);
            triMesh.uvTris[i].uvs[j].x[1] = UV(uv_idx, 1);
        }
    }
